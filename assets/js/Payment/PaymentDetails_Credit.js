...
const paymentFormElements = {
    ...
    billNumberInput: document.getElementById("invoiceNumberInput")
};

const accountedAmountInput = document.getElementById("accountedAmount");
const billBalanceInput = document.getElementById("invoiceBalance");
const paymentAmountInput = document.getElementById("paymentAmount");

const billDate = document.getElementById("invoiceDate");
const billAmount = document.getElementById("invoiceAmount");

const addBillDetailsButton = document.getElementById("addInvoiceDetailsButton");
...
async function getPendingInvoiceDetails(partyCode) {
    ...
    allInvoices.forEach(bill => {
        invoiceMap[bill.InvoiceNo] = bill;
    });
...
function refreshBillDatalist() {
    ...
    allInvoices.forEach(bill => {
        ...
        if (!addedInvoices.has(bill.InvoiceNo)) {

            const option = document.createElement("option");
            option.value = `${bill.InvoiceNo}`;
...
document.getElementById("invoiceNumberInput").addEventListener("change", function () {
    ...
    const bill = invoiceMap[selectedValue];

    if (!bill) return;

    document.getElementById("invoiceDate").value =
        bill.InvoiceDate
            ? bill.InvoiceDate.split("T")[0]
            : "";

    document.getElementById("invoiceAmount").value =
        Number(bill.GrandTotalAmount || 0).toFixed(2);
...
function validateAccountedAmount() {
    ...
    const otherDeduction = safeNumber(
        document.getElementById("otherDeuctionAmount").value // FIXED: Corrected ID
    );
    const tdsDeduction = safeNumber(
        document.getElementById("tDSDeuctionAmount").value // FIXED: Corrected ID
    );
...
// ADD BILL ROW
// ------------------------------------------
document.getElementById("addInvoiceDetailsButton").addEventListener("click", () => {

    const selectedBill = document.getElementById("invoiceNumberInput").value.trim();
    ...
    const billInfo = invoiceMap[selectedBill];
    ...
    const otherDeduction = safeNumber(document.getElementById("otherDeuctionAmount").value); // FIXED: Corrected ID
    const tdsDeduction = safeNumber(document.getElementById("tDSDeuctionAmount").value); // FIXED: Corrected ID
...
function clearBillInputs() {
    ...
    document.getElementById("otherDeuctionAmount").value = "0.00"; // FIXED: Corrected ID
    document.getElementById("tDSDeuctionAmount").value = "0.00"; // FIXED: Corrected ID
        ...
        document.querySelectorAll("#paymentDetails tbody tr")
            .forEach(row => {
                ...
                lineItems.push({
                    ...
                    Narration: row.cells[2].innerText.trim(),
...
modifyButton.addEventListener("click", function () {
    ...
    // Enable invoice inputs - FIXED: Corrected IDs
    [
        "invoiceNumberInput",
        "accountedAmount",
        "otherDeuctionAmount",  // FIXED: Corrected ID
        "tDSDeuctionAmount",   // FIXED: Corrected ID
        "narration"
    ].forEach(id => {
