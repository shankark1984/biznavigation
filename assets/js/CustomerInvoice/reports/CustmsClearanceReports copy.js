async function generate_CustmsClearance_InvoicePDF(header, lines = []) {
    ...
    /* ==============================
       SHIPMENT TABLE
    ============================== */
    let totalFreightAmt = 0, totalGstAmt = 0, totalGrandTotal = 0;
    ...
    const tableData = Array.from(table.querySelectorAll("tbody tr")).map((tr, i) => {
        ...
        const freightAmt = safeNumber(c[15]?.innerText);
        const gstAmt = safeNumber(c[19]?.innerText);
        const grandTotal = safeNumber(c[20]?.innerText);
        ...
        return [
            i + 1, // Sl No
            formatDate(c[2]?.innerText) || "", // Date
            c[3]?.innerText || "", // BL / AWB No
            formatDate(c[4]?.innerText) || "", // BL / AWB Date
            c[5]?.innerText || "",// BE No
            formatDate(c[6]?.innerText) || "", // BE date
            c[7]?.innerText || "", // Movement Type
            c[8]?.innerText || "", // Transit Type
            c[9]?.innerText || "", // Mode Type
            c[11]?.innerText || "", // Clearance Port
            c[13]?.innerText || "", // Quantity
            c[14]?.innerText || "", // Weight
            c[15]?.innerText || "", // Amount
            c[19]?.innerText || "", // GST Amount
            c[20]?.innerText || "" // Grand Total
        ];
    });

    const shipmentGrandTotal = totalFreightAmt + totalGstAmt + totalGrandTotal;
    ...
    doc.autoTable({
        ...
        head: [[
            "Sl", "Date", "BL / AWB No", "BL / AWB Date", "BE No",
            "BE Date", "Movement Type", "Transit Type", "Mode Type", "Clearance Port",
            "Quantity", "Weight (Kgs)", "Amount", "GST Amount", "Grand Total"
        ]],
        body: tableData,
        styles: {
        ...
        foot: [
            [
                ...
                { content: totalFreightAmt.toFixed(2), styles: { halign: "right", fontStyle: "bold" } },
                { content: totalGstAmt.toFixed(2), styles: { halign: "right", fontStyle: "bold" } },
                { content: totalGrandTotal.toFixed(2), styles: { halign: "right", fontStyle: "bold" } },
    ...
    // Tax summary rows
    const rowsData = [
        ["Total Freight", totalFreightAmt],
        ["Fuel Charges", totalGstAmt],
        ["Other Charges", totalGrandTotal],
        ["Sub Total", taxable],
        ["CGST @ 9%", cgst],
        ["SGST @ 9%", sgst],
        ["IGST @ 18%", igst],
